...
namespace App\Services\Sdui\Scenes\Legacy;

use App\Models\AuthUser;
use App\Models\OperasiPenugasan;
use App\Services\Auth\AuthorizationContextService;
use App\Services\Sdui\Scenes\SceneComposer;
...
class AkunSceneComposer implements SceneComposer
{
    ...
    private function fetchCommandCenter(): ?array
    {
        ...
        $scopeFilter = $this->ctx->hasRole('pcnu') ? "AND id_pcnu = " . (int)$this->ctx->getScopeId() : "";
        $res = DB::select("
            SELECT id_insiden, kode_kejadian, status_insiden, command_state, prioritas, lama_kejadian_hari, nama_klaster, jumlah_sitrep
            FROM v_command_center_summary
            WHERE status_insiden NOT IN ('selesai', 'dibatalkan')
            $scopeFilter
            ORDER BY FIELD(prioritas, 'kritis', 'tinggi', 'sedang', 'rendah'), lama_kejadian_hari DESC
            LIMIT 5
        ");
        return array_map(fn($item) => (array) $item, $res);
    }
    ...
    private function fetchAlertInsiden(): ?array
    {
        ...
        $scopeFilter = $this->ctx->hasRole('pcnu') ? "WHERE id_pcnu = " . (int)$this->ctx->getScopeId() : "";
        $res = DB::select("
            SELECT kode_kejadian, nama_bencana, nama_pcnu, waktu_mulai, prioritas
            FROM v_alert_insiden_baru
            $scopeFilter
            LIMIT 3
        ");
        return array_map(fn($item) => (array) $item, $res);
    ...
    {
        return md5($this->computeVersion($profil) . '_' . $this->user->id_pengguna);
    }
    ...
    private function buildIdentityCard(?array $profil, ?array $jabatanAktif, array $keahlian): array
    {
        if (!$profil) {
            return [
                ...
                'children' => [
                    ['type' => 'text', 'content' => 'Anda belum login', 'style' => ['color' => 'white']],
                    ['type' => 'spacer', 'style' => ['height' => '4']],
    ...
    {
        if ($commandCenter === null) return null; // not authorized role
                ...
                $items[] = [
                    ...
                    'children' => [
                        ...
                        [
                            ...
                            'style' => ['gap' => '2', 'align' => 'center'],
                            'children' => [
                                ['type' => 'icon', 'name' => 'group', 'style' => ['size' => 13, 'color' => $c['nama_klaster'] === 'Tunggu Aktivasi' ? 'yellow-500' : 'green-600']],
                                ['type' => 'text', 'content' => $c['nama_klaster'], 'style' => ['size' => 'xs', 'color' => $c['nama_klaster'] === 'Tunggu Aktivasi' ? 'yellow-600' : 'green-700', 'weight' => $c['nama_klaster'] === 'Tunggu Aktivasi' ? 'medium' : 'normal']]
                            ]
                        ],
                        ['type' => 'spacer', 'style' => ['height' => '3']],
                        ['type' => 'divider'],
                        ...
                        [
                            ...
                            'children' => [
                                [
                                    ...
                                    'style' => ['align' => 'center'],
                                    'children' => [
                                        ['type' => 'text', 'content' => (string)$c['lama_kejadian_hari'], 'style' => ['family' => 'mono', 'size' => 'xl', 'weight' => 'bold', 'color' => $c['lama_kejadian_hari'] > 7 ? 'red-600' : 'gray-900']],
                                ...
                                [
                                    ...
                                    'style' => ['align' => 'center'],
                                    'children' => [
                                        ['type' => 'text', 'content' => (string)$c['jumlah_sitrep'], 'style' => ['family' => 'mono', 'size' => 'xl', 'weight' => 'bold', 'color' => 'gray-900']],
    ...
    private function buildAkunMenu(?array $profil): ?array
    {
        ...
        $am = $this->getStatusAkunMapping($profil['status_akun']);
