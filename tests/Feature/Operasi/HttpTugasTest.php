...
class HttpTugasTest extends TestCase
{
    ...
    public function test_can_create_tugas_success()
    {
        ...
        $response = $this->actingAs($user)->postJson(route('api.operasi.tugas.store'), [
            'id_operasi_klaster' => $klaster->id_klaster_operasi,
            'judul_tugas' => 'Membagikan logistik beras',
        ]);

        $response->assertStatus(201)
                 ->assertJsonPath('data.judul', 'Membagikan logistik beras')
                 ->assertJsonMissingPath('data.judul_tugas');

        $this->assertDatabaseHas('operasi_tugas', [
            'id_operasi_klaster' => $klaster->id_klaster_operasi,
            'judul_tugas' => 'Membagikan logistik beras',
            'status_tugas' => 'rencana' // default
        ]);
    }

    public function test_start_tugas_success()
    {
        $user = $this->mockUser('pcnu', 'pcnu', 1);
        $insiden = $this->createInsiden(1);
        $klaster = OperasiKlaster::create([
        ...
        $response = $this->actingAs($user)->postJson(route('api.operasi.tugas.start', $tugas));

        $response->assertStatus(200)
                 ->assertJsonPath('data.status', 'berjalan')
                 ->assertJsonMissingPath('data.status_tugas');

        $this->assertEquals('berjalan', $tugas->fresh()->status_tugas);
    }

    public function test_complete_tugas_success()
    {
        $user = $this->mockUser('pcnu', 'pcnu', 1);
        $insiden = $this->createInsiden(1);
        $klaster = OperasiKlaster::create([
        ...
        $tugas = OperasiTugas::create([
            'id_operasi_klaster' => $klaster->id_klaster_operasi,
            'judul_tugas' => 'Beras 1',
            'status_tugas' => 'berjalan', // harus berjalan
        ]);

        $response = $this->actingAs($user)->postJson(route('api.operasi.tugas.complete', $tugas));

        $response->assertStatus(200)
                 ->assertJsonPath('data.status', 'selesai')
                 ->assertJsonMissingPath('data.status_tugas');

        $this->assertEquals('selesai', $tugas->fresh()->status_tugas);
    }
}
