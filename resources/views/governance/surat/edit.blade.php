<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">Edit Surat: {{ $surat->nomor_surat_resmi }}</h2>
            <a href="{{ route('surat.show', $surat) }}" class="text-sm text-gray-600 hover:text-gray-900">&larr; Kembali</a>
        </div>
    </x-slot>
                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    <form method="POST" action="{{ route('surat.update', $surat) }}" class="space-y-6">
                        ...
                        <div>
                            ...
                            <select name="id_jenis_surat" id="id_jenis_surat" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                ...
                                @foreach($jenisSurat as $jns)
                                    <option value="{{ $jns->id_jenis_surat }}" {{ old('id_jenis_surat', $surat->id_jenis_surat) == $jns->id_jenis_surat ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <input type="text" name="perihal" id="perihal" required maxlength="255" value="{{ old('perihal', $surat->perihal) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                        ...
                        <div>
                            ...
                            <input type="date" name="tgl_terbit" id="tgl_terbit" required value="{{ old('tgl_terbit', $surat->tgl_terbit?->format('Y-m-d')) }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                        ...
                        <div>
                            ...
                            <select name="id_pengguna_ttd" id="id_pengguna_ttd" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                ...
                                @foreach($pengguna as $user)
                                    <option value="{{ $user->id_pengguna }}" {{ old('id_pengguna_ttd', $surat->id_pengguna_ttd) == $user->id_pengguna ? 'selected' : '' }}>
                        ...
                        <div>
                            <label for="id_jabatan_ttd" class="block text-sm font-medium text-gray-700">Jabatan Penandatangan</label>
                            <select name="id_jabatan_ttd" id="id_jabatan_ttd" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                ...
                                @foreach($jabatan as $jbt)
                                    <option value="{{ $jbt->id_jabatan }}" {{ old('id_jabatan_ttd', $surat->id_jabatan_ttd) == $jbt->id_jabatan ? 'selected' : '' }}>
                        ...
                        <div>
                            <label for="id_insiden" class="block text-sm font-medium text-gray-700">Insiden Terkait</label>
                            <select name="id_insiden" id="id_insiden" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                ...
                                @foreach($insidenList as $ins)
                                    <option value="{{ $ins->id_insiden }}" {{ old('id_insiden', $surat->id_insiden) == $ins->id_insiden ? 'selected' : '' }}>
                        ...
                        <div class="flex items-center justify-end space-x-3">
                            <a href="{{ route('surat.show', $surat) }}" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                                Batal
                            </a>
