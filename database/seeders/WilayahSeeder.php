...
class WilayahSeeder extends Seeder
{
    ...
    {
        // 1. Seed wilayah_kabupaten (35 data Jawa Tengah)
        ...
        // 2. Seed wilayah_kecamatan dari file SQL eksternal (skip jika sudah ada)
        if (WilayahKecamatan::count() === 0) {
            $sqlFile = database_path('sql/wilayah_kecamatan.sql');
            if (file_exists($sqlFile)) {
                DB::unprepared(file_get_contents($sqlFile));
            }
        }

        // 3. Seed wilayah_desa dari file SQL eksternal (skip jika sudah ada)
        if (WilayahDesa::count() === 0) {
            $sqlFile = database_path('sql/wilayah_desa.sql');
            if (file_exists($sqlFile)) {
                DB::unprepared(file_get_contents($sqlFile));
            }
        }
